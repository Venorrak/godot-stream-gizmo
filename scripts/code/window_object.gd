...
func setContent(info : Dictionary) -> void:
	if contentRef.has_method("setContent"):
		contentRef.setContent(info)
	else:
		push_error("no setContent method")
